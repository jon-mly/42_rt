...
/*   Updated: 2019/02/14 23:38:24 by guillaume        ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	attach_widgets_grid(GtkWidget **lbls, GtkWidget *grid, guint size)
{
	...
	while (++i < size)
	{
		(i <= 7) ?
			...
			gtk_grid_attach(GTK_GRID(grid), GTK_WIDGET(lbls[i]), 1, i - 8, 1, 1);
	}
}
...
void	init_size_w(GtkWidget **size_buttons, GtkWidget *w_size_grid, t_settings *rt_set)
{
	GSList *rb;
	...
	rb = gtk_radio_button_get_group(GTK_RADIO_BUTTON(size_buttons[0]));
	while (rb)
	{
...
void	init_server_w(GtkWidget **server_w, GtkWidget *server_grid, t_settings *rt_set)
{
